...
w 	:- 	playerPosition([X,Y]), A is X+1, isDungeonPosition(A,Y),
		...
		write('-->>> You face Dungeon Boss <<<--'),nl,nl, asserta(theMusuh(boss)),
		statusEnemy(boss),foundDungeon,nl,nl,
		write('-->>> attack / run '),!.
w 	:- 	isPlay,playerPosition([X,Y]), A is X+1, isStorePosition(A,Y),
		nl, write('~~~~~~~ You move to North ~~~~~~~'),nl,
		nl, write('-->>> Welcome to NyeNye Store !'),nl,
		write('-->>> Access store by \'shop.\''),
		A is X+1, 
		B = Y, 
		retract(playerPosition([X,Y])),
...
a 	:- 	isPlay,playerPosition([X,Y]), B is Y+1, isStorePosition(X,B),
		...
		retract(playerPosition([X,Y])),
		asserta(playerPosition([A,B])),!.
a 	:- 	isPlay,playerPosition([X,Y]), B is Y+1, isQuestPosition(X,B),
		nl, write('~~~~~~~ You move to West ~~~~~~~'),nl,
		getQuest,
		B is Y+1, 
		A = X, 
...
s 	:- 	isPlay,playerPosition([X,Y]), A is X-1, isStorePosition(A,Y),
		...
		retract(playerPosition([X,Y])),
		asserta(playerPosition([A,B])),!.
s 	:- 	isPlay,playerPosition([X,Y]), A is X-1, isQuestPosition(A,Y),
		nl, write('~~~~~~~ You move to South ~~~~~~~'),nl,
		getQuest,
		A is X-1, 
		B = Y, 
...
d 	:- 	isPlay,playerPosition([X,Y]), B is Y-1, isDungeonPosition(X,B),
		...
		statusEnemy(boss),foundDungeon,nl,nl,
		write('-->>> attack / run '),!.
d 	:- 	isPlay,playerPosition([X,Y]), B is Y-1, isStorePosition(X,B),
		nl, write('~~~~~~~ You move to East ~~~~~~~'),nl,
		nl, write('-->>> Welcome to NyeNye Store !'),nl,
		write('-->>> Access store by \'shop.\''),
		asserta(getStore),
		...
		retract(playerPosition([X,Y])),
		asserta(playerPosition([A,B])),!.
d 	:- 	isPlay,playerPosition([X,Y]), B is Y-1, isQuestPosition(X,B),
		nl, write('~~~~~~~ You move to East ~~~~~~~'),nl,
		getQuest,
		B is Y-1, 
		A = X, 
...
/* Menghapus musuh yang telah dikalahakan (berada pada posisi yang sama dengan player). */
makeMusuhMati	:-	playerPosition([X,Y]), 
					musuh(_C,[X1,Y1]), 
					X=:=X1, 
					Y=:=Y1, 
					retract(musuh(_C,[X1,Y1])),!.
...
teleportD	:-	isPlay,
				...
				dungeonPosition([X1,Y1]),
				asserta(playerPosition([X1,Y1])),
				nl,nl,write('-->>> You face Dungeon Boss <<<--'),nl,nl,asserta(theMusuh(boss)),
				statusEnemy(boss),foundDungeon,nl,nl,
				write('-->>> attack / run '),!.
